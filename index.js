...
export const CssVarsPonyfill = (params) => {
  if(isNativeSupport)
    return;
  ...
  if(includeExternal){
    const cssText = document.styleSheets && document.styleSheets.map(sheet => {
      if(sheet && sheet.cssRules && sheet.cssRules.length){
        return sheet.cssRules.map(rule => rule && rule.cssText).join("")
      }
      return ''
    }).join("")
    if(cssText)
      minified += cssText.replace(/[\n][\s]+/g, "")
  }
